...
const IdeasPageNew = () => {
    ...
    const handleWordCount = (e, i, max) => {
        const obj = { i, count: (max || 100) - e.target.value.length }
        const newItems = [...wordCount]
        const findExistanceIndex = newItems.findIndex((item) => item?.i == i)
        if (findExistanceIndex === -1) {
            newItems.push(obj)
        } else {
            const temp = newItems[findExistanceIndex]
            newItems[findExistanceIndex] = {
                ...temp,
                count: (max || 100) - e.target.value.length
            }
        }
    ...
    const handleChange = (e) => {
        const newItems = [...answerResponses]
        const obj = {
        ...
        if (findExistanceIndex === -1) {
            ...
            if (e.target.type === 'checkbox') {
                const options = [...temp.selected_option]
                const indexOfCheckedAnswers = options.indexOf(e.target.value)
    ...
    const fileHandler = (e, id) => {
        ...
        e.target.files = null
        const pattern = /^[a-zA-Z0-9_-\s]{0,}$/
        const checkPat = choosenFiles.filter((item) => {
            const pat = item.name.split('.')
            pat.pop()
